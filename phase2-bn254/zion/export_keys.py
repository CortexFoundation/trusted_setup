...
def mpc_export_keys(circuit):
    ...
    subprocess.check_call(
        [
            config.phase2_repo_path + "phase2/target/release/export_keys",
            params,
            bellman_vk,
            bellman_pk,
        ]
    ...
    subprocess.check_call(
        [
            "python3",
            config.phase2_repo_path + "phase2/tools/vk2ethsnarks.py",
            bellman_vk,
            vk,
        ]
    )
    ...
    subprocess.check_call([config.tool_executable, "-createpk", bellman_pk, pk])
